...
class FeedForward(nn.Module):
    def __init__(self, dim, mult = 2, dropout = 0.3):
        super().__init__()
...
class SafeCrossAttnEncoder(nn.Module):
    ...
    包含了防 NaN 的安全机制。
    """
    def __init__(self, embed_dim: int, num_heads: int = 8, dropout: float = 0.3, ffn_mult: int = 2):
        super().__init__()
