...
class TCGA_BRCA_Dataset(MultiModalDataset):
    ...
    def __init__(self, args, mode: str = "train", modalities: str = "all", fold: int = None):
        ...
        if args.knowledge_type == 'all':
            self.dataset_dir = os.path.join(project_root, "Data", "TCGA-BRCA")
        elif args.knowledge_type == 'relationship':
            self.dataset_dir = os.path.join(project_root, "Data-Correlation", "TCGA-BRCA")
        elif args.knowledge_type == 'survival':
            self.dataset_dir = os.path.join(project_root, "Data-Survival", "TCGA-BRCA")
        else:
        ...
        split_file = os.path.join(self.processed_dir, "brca_patients_5fold.json") 
    ...
    def _load_tabular_and_labels(self):
        try:
            ...
            self.labels_df = pd.read_csv(os.path.join(self.processed_dir, "brca_patient_labels.csv"), dtype=str)
        except Exception as e:
        ...
        if "tabular-clinical-8" in self.modalities:
        ...
        if "tabular-treatment-15" in self.modalities:
