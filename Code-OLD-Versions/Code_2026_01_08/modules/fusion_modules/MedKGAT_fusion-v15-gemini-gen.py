...
class MedKGATFusion(nn.Module):
    ...
    def _compute_kl_loss(
        ...
        pred_logits = pred_sims * temperature  # (B, E)

        # Mask invalid edges for prediction distribution (like Version1)
        INF_MASK_VAL = -1e4
        mask_adder = (1.0 - valid_mask) * INF_MASK_VAL  # (B, E)

        # Pred log-probs over edges per patient
        pred_log_probs = F.log_softmax(pred_logits + mask_adder, dim=1)  # (B, E)
        target_probs = F.softmax(target_scores, dim=1)  # (B, E)

        # KL divergence per edge, then sum per patient
        kl_loss = F.kl_div(pred_log_probs, target_probs, reduction="none")  # (B, E)
        kl_loss_per_patient = kl_loss.sum(dim=1)  # (B,)
        valid_patients = (valid_mask.sum(dim=1) > 1).float()  # (B,)
        final_loss = (kl_loss_per_patient * valid_patients).sum() / valid_patients.sum().clamp(min=1.0)
