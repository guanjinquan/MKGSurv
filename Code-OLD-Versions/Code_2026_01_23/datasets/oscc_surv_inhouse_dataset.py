...
class OSCCSurvInHouseDataset(MultiModalDataset):
    ...
    def __init__(self, args, mode="train", modalities="all", fold=None):
        ...
        if args.knowledge_type == 'all':
            self.dataset_dir = os.path.join(project_root, "Data", "Multi-OSCCPI-Dataset")
        elif args.knowledge_type == 'relationship':
            self.dataset_dir = os.path.join(project_root, "Data-Correlation", "Multi-OSCCPI-Dataset")
        elif args.knowledge_type == 'survival':
            self.dataset_dir = os.path.join(project_root, "Data-Survival", "Multi-OSCCPI-Dataset")
        else:
        ...
        if self.args.use_medical_knowledge:
            ...
            for k, v in kdata.items():
                if "knowledge" in v:
                    ...
                    raise ValueError("Invalid knowledge data")
        else:
            kdata = self.knowledge_dict.get(pid_str, None)
            output_dict["medical-knowledge"] = {}
