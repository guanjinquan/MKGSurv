...
class SafeCrossAttnEncoder(nn.Module):
    ...
    def __init__(self, embed_dim: int, num_heads: int = 8, dropout: float = 0.1, ffn_mult: int = 4, use_moe: bool = False):
        ...
        if use_moe:
            ...
            self.ffn = ExpertFeedForward(embed_dim, mult=ffn_mult, dropout=dropout)
...
class EdgeContextualizer(nn.Module):
    ...
    def __init__(self, embed_dim: int, num_heads: int = 4):
        super().__init__()
        self.cross_attn = SafeCrossAttnEncoder(embed_dim, num_heads, use_moe=True)
...
class MedKGATFusion(nn.Module):
    def __init__(self, args, embed_dim: int, 
        ...
        self.shared_inter_layer = nn.ModuleDict({
            'edge_to_node_attn': SafeCrossAttnEncoder(embed_dim, num_heads=8, dropout=attn_dropout_rate, use_moe=True),
            'node_to_node_attn': SafeCrossAttnEncoder(embed_dim, num_heads=8, dropout=attn_dropout_rate, use_moe=True),
