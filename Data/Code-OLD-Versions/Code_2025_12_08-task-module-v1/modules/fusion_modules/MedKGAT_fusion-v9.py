...
class MedKGATFusion(nn.Module):
    ...
    def forward(
        ...
        batch_cls_tokens = self.cls_token.expand(batch_size, -1, -1).to(embeddings[0].device)
        batch_cls_masks = torch.ones(batch_size, 1, device=embeddings[0].device)
        global_concat = torch.cat([batch_cls_tokens] + final_group_embeddings, dim=1)
        global_mask = torch.cat([batch_cls_masks] + group_masks, dim=1)
        ...
        fused_embedding = global_transformed[:, 0]
