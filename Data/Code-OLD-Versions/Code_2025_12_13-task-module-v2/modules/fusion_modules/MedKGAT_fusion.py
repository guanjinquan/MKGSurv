...
--- Testing Complete ---
Validation Summary:
C-Index_Validation Set: 0.6473 ± 0.0550
 - List = [0.6799387442572741, 0.5640219952867243, 0.6083803384367445, 0.6635145784081954, 0.7205067300079177]
C-Index-IPCW_Validation Set: 0.6209 ± 0.0753
 - List = [0.7181231372004871, 0.5187241212907449, 0.5749418892951842, 0.5960184560123956, 0.6966394133653494]
Test Summary:
C-Index_Test Set: 0.6357 ± 0.0482
 - List = [0.6049573973663827, 0.6545961002785515, 0.7144654088050314, 0.6327493261455526, 0.5715277777777777]
C-Index-IPCW_Test Set: 0.6203 ± 0.0442
 - List = [0.5993677662530336, 0.7034050934454821, 0.5916866834100301, 0.626309777839915, 0.5807637520149127]
Training run tcga_luad_run006 finished.
"""
import sys
import os
...
class MedKGATFusion(nn.Module):
    ...
    def _contrastive_loss(self, features_a: torch.Tensor, features_b: torch.Tensor) -> torch.Tensor:
        ...
        # 限制 logit_scale 的范围，防止梯度爆炸 (可选，但在医疗数据这种稀疏场景下推荐)
    ...
    def forward(
        ...
        intra_cl_loss = self._compute_intra_cl_loss(info_level_embeddings, masks, embeddings_groups)
        ...
        return {
            ...
            "loss_dict": {
                "total_loss": 2 * fusion_loss + intra_cl_loss,
