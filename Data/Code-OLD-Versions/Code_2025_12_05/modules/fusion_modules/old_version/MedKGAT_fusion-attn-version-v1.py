...
class MedKGATFusion(nn.Module):
    def __init__(self, args, embed_dim: int, max_modalities: int = 10, dropout_rate: float = 0.25, 
            num_intra_layers: int = 2, num_inter_layers: int = 2):
        super().__init__()
        ...
        for _ in range(num_inter_layers):
            self.inter_layers.append(nn.ModuleDict({
                'edge_to_node_attn': SafeCrossAttentionBlock(embed_dim, num_heads=8),
                'node_to_node_attn': SafeCrossAttentionBlock(embed_dim, num_heads=8),
