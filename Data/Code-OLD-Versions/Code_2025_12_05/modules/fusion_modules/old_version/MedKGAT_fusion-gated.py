...
class MedKGATFusion(nn.Module):
    def __init__(self, args, embed_dim: int, 
        ...
        # 2. Intra-group Interaction (组内交互)
        # 处理同一个组（如同一个模态或同一个概念）内部的特征交互
        ...
        # 3. GAT Interaction Components (Inter-Group / 组间交互)
        # 包含了边更新、节点查询边的注意力、节点更新节点的注意力
        ...
        # [NEW] Aggregation Scorer (聚合评分器)
        # 用于计算来自邻居的 Update 向量的重要性分数
        # 输入: [Batch, Seq, Dim], 输出: [Batch, Seq, 1]
        self.aggregation_scorer = nn.Sequential(
            nn.Linear(embed_dim, embed_dim // 2),
            nn.Tanh(),
            nn.Linear(embed_dim // 2, 1)
        )

        # 4. Global Aggregation (全局聚合)
    ...
    def _intra_group_step(self, embeddings: List[torch.Tensor], masks: List[torch.Tensor], groups: List[List[int]]) -> List[torch.Tensor]: 
        ...
        for group_indices in groups:
            ...
            # Safe Transformer Check: 防止全 mask 导致 NaN
    ...
    def _inter_group_step(self, target_node: torch.Tensor, target_mask: torch.Tensor,
        ...
        """
        单向交互: Source -> Edge -> Target
        这里计算出的 output 是 "Source 对 Target 的建议更新值"
        """
        source_padding_mask = (source_mask == 0)
        edge_padding_mask = (edge_mask == 0)
        
        # Step 1: Edge queries Source (Gating mechanism)
        ...
        # 注意: 这里计算的是 "update"，还不是最终结果，需要后续聚合
    ...
    def forward(
        ...
        # 4. 组间交互 (Inter-Group Interaction with Attention Aggregation)
        ...
        for layer_idx in range(self.num_inter_layers):
            ...
            # Buffer 现在存储 (Update_Vector, Attention_Score, Mask)
            group_updates_buffer = [[] for _ in range(num_groups)]
            
            next_proj_knowledge = {} 
            
            # --- 遍历所有边计算 Update 和 Score ---
            for (idx_a, idx_b), edge_feat in current_proj_knowledge.items():
                ...
                # [New] 计算分数
                score_b = self.aggregation_scorer(update_for_b) # [B, L, 1]
                group_updates_buffer[idx_b].append((update_for_b, score_b, weight_for_b))
                ...
                # [New] 计算分数
                score_a = self.aggregation_scorer(update_for_a) # [B, L, 1]
                group_updates_buffer[idx_a].append((update_for_a, score_a, weight_for_a))

            # --- 聚合阶段 (Attention Weighted Aggregation) ---
            next_group_embeddings = []
            for i in range(num_groups):
                original_group_feat = current_group_embeddings[i]
                updates_data = group_updates_buffer[i] # List of (update, score, weight)

                if len(updates_data) > 0:
                    # 解包
                    updates = [item[0] for item in updates_data] # List of [Batch, L, Dim]
                    scores  = [item[1] for item in updates_data] # List of [Batch, L, 1]
                    weights = [item[2] for item in updates_data] # List of [Batch, 1, 1]

                    # 堆叠: 维度 0 变成 Neighbor 维度
                    # [Num_Neighbors, Batch, Seq_Len, Dim]
                    stacked_updates = torch.stack(updates, dim=0)
                    # [Num_Neighbors, Batch, Seq_Len, 1]
                    stacked_scores = torch.stack(scores, dim=0)
                    # [Num_Neighbors, Batch, 1, 1] -> 广播到 [..., Seq_Len, 1]
                    stacked_weights = torch.stack(weights, dim=0)
                    
                    # 1. Masking Scores: 
                    # 将无效邻居 (weight=0) 的分数设为负无穷，使其 Softmax 后权重为 0
                    stacked_scores = stacked_scores.masked_fill(stacked_weights == 0, -1e9)
                    
                    # 2. Attention Weights (Softmax over neighbors)
                    # dim=0 是邻居维度
                    attn_weights = F.softmax(stacked_scores, dim=0) # [Num_N, B, L, 1]
                    
                    # 3. Weighted Sum
                    # 处理 NaN: 如果所有邻居都是无效的 (all weights=0)，Softmax 结果可能是均匀分布或NaN
                    # 这里再次乘以 stacked_weights 确保安全性 (0 * anything = 0)
                    weighted_updates = stacked_updates * attn_weights * stacked_weights
                    aggregated_feat = torch.sum(weighted_updates, dim=0) 
                    
                    # 4. Residual Connection (残差连接) !!! 重要 !!!
                    # 新特征 = 原特征 + 邻居聚合特征
                    next_feat = original_group_feat + aggregated_feat
                    
                    next_group_embeddings.append(next_feat)
                else:
                    # 没有邻居更新，保持原样
        ...
        # ------------------------------------------------------------------
        # 4b. 计算辅助 Loss (KL Loss)
        # 保持原逻辑不变，用于约束相似度和知识边的关系
        # ------------------------------------------------------------------
