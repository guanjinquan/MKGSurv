...
import json
from modules.fusion_modules.healnet_fusion import HealNet
...
class MedKGATFusion(nn.Module):
    def __init__(self, args, embed_dim: int, max_modalities: int = 10, num_groups: int = 4, dropout_rate: float = 0.1, 
        ...
        self.healnet = HealNet(
            n_modalities=num_groups,
            # Each input is (b, n, d), treated as 1 spatial axis 'n' and channel dim 'd'
            channel_dims=[self.embed_dim] * num_groups,
            num_spatial_axes=[1] * num_groups,
            out_dims=self.embed_dim, # Final output feature dimension
            l_c=512,    # Number of latent vectors (queries)
            l_d=self.embed_dim,      # Dimension of latent vectors
            depth=1,
            x_heads=8,
            l_heads=8,
            ff_dropout=0.1,
            attn_dropout=0.1,
            fourier_encode_data=False # Embeddings are pre-computed, no need for fourier encoding
        )
    ...
    def forward(
        ...
        for layer_idx in range(self.num_inter_layers):
            ...
            # Apply Updates to Groups (Aggregation)
        ...
        fused_embedding = self.healnet(tensors=final_group_embeddings, masks=group_masks)    
