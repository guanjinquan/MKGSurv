...
--- Testing Complete ---
Validation Summary:
C-Index_Validation Set: 0.6263 ± 0.0748
 - List = [0.6059629331184528, 0.5011783189316575, 0.6193853427895981, 0.6904196357878069, 0.7143950995405819]
C-Index-IPCW_Validation Set: 0.6122 ± 0.0814
 - List = [0.5754713144083097, 0.5191257579616309, 0.5549841458693667, 0.6716964756052771, 0.7397928431122162]
Test Summary:
C-Index_Test Set: 0.6445 ± 0.0700
 - List = [0.7220125786163522, 0.6713091922005571, 0.6853099730458221, 0.625, 0.5189775367931836]
C-Index-IPCW_Test Set: 0.6094 ± 0.0679
 - List = [0.63796167564641, 0.6936168889334984, 0.6351366816971156, 0.5895201748389298, 0.49083516814486267]
...
class MedKGATFusion(nn.Module):
    ...
    def _compute_intra_sinkhorn_loss(self, 
        ...
        for group_indices in groups:
            ...
            for i in range(num_modalities):
                for j in range(i + 1, num_modalities):
                    ...
                    # 计算 Sinkhorn Distance
                    pair_loss = self._sinkhorn_distance(feat_i_clean, feat_j_clean)
    ...
    def forward(
        ...
        intra_cl_loss = self._compute_intra_sinkhorn_loss(info_level_embeddings, masks, embeddings_groups)
        ...
        return {
            ...
            "loss_dict": {
                "total_loss": 2 * fusion_loss + 0.5 * intra_cl_loss,
                "KL_Loss": fusion_loss,
