...
--- Testing Complete ---
Validation Summary:
C-Index_Validation Set: 0.6338 ± 0.0757
 - List = [0.6188557614826753, 0.5113904163393559, 0.6138691883372734, 0.6943784639746635, 0.7304747320061256]
C-Index-IPCW_Validation Set: 0.6309 ± 0.0859
 - List = [0.5928745200430029, 0.5144616837139009, 0.5911749111203632, 0.7011492473716038, 0.7547510526471931]
Test Summary:
C-Index_Test Set: 0.6151 ± 0.0513
 - List = [0.6138364779874214, 0.6685236768802229, 0.648921832884097, 0.6243055555555556, 0.5197521301316809]
C-Index-IPCW_Test Set: 0.5965 ± 0.0718
 - List = [0.5605429260155944, 0.6993333932950353, 0.633393877548261, 0.6045977056770415, 0.48481148118149137]
Training run tcga_luad_run008 finished.
"""
import sys
import os
...
class MedKGATFusion(nn.Module):
    ...
    def _barlow_twins_loss(self, z1: torch.Tensor, z2: torch.Tensor) -> torch.Tensor:
        """
        计算 Barlow Twins Loss。
        z1, z2: (Batch, Dim) - 必须是归一化后的特征
        """
        batch_size = z1.shape[0]
        # 如果有效 batch 太小，无法计算统计量，返回 0
        if batch_size < 2: 
            return torch.tensor(0.0, device=z1.device)

        # 1. 归一化 (Barlow Twins 对 BN 很敏感，这里用简单的 std 归一化)
        # 加上 1e-6 防止除以 0
        ...
        # 3. Loss = (对角线 - 1)^2 + lambda * (非对角线)^2
        # Lambda 通常取 0.001
        on_diag = torch.diagonal(c).add_(-1).pow_(2).mean()
        off_diag = off_diagonal(c).pow_(2).mean() 
        
        loss = on_diag + 0.001 * off_diag
        return loss 

    def _compute_intra_barlow_loss(self, 
        ...
        for group_indices in groups:
            ...
            for i in range(num_modalities):
                for j in range(i + 1, num_modalities):
                    ...
                    # 提取有效子集 (Subset Slicing) - 只有两个模态都存在的样本才参与计算
                    feat_i_clean = group_pooled_feats[i][valid_indices]
                    feat_j_clean = group_pooled_feats[j][valid_indices]
                    
                    # 计算 Barlow Loss
                    pair_loss = self._barlow_twins_loss(feat_i_clean, feat_j_clean)
    ...
    def forward(
        ...
        intra_loss = self._compute_intra_barlow_loss(info_level_embeddings, masks, embeddings_groups)
