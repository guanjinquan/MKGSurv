...
class TCGA_BRCA_SurvivalPred(nn.Module):
    ...
    def __init__(
        ...
        if 'image-pathology' in self.active_modalities:
            ...
            self.image_proj = nn.Sequential(
                ...
                SNN_Block(image_input_dim, self.embed_dim),
            ...
            if "tabular" in mod_name:
                try:
                    ...
                    self.tabular_encoders[mod_name] = nn.Sequential(
                        SNN_Block(in_dim, self.embed_dim),
                        nn.LayerNorm(self.embed_dim),
    ...
    def _pad_and_mask_modality(self, data_list: List[Optional[torch.Tensor]]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        assert input_dim is not None, f"No input dimension found, got {len(data_list)}"
        assert input_dim > 0, f"Input dimension must be greater than 0."
        assert max_seq_len > 0, f"Max sequence length must be greater than 0."
        ...
        for i, t in enumerate(processed_list):
            if t is not None:
                length = t.shape[0]
                assert length > 0, f"Empty tensor found in batch."
                assert t.shape[1] > 0, f"Dim of tensor must be greater than 0."
                # Fill data
