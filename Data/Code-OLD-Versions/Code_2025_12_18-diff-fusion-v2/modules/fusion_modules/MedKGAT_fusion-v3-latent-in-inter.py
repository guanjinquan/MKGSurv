...
class MedKGATFusion(nn.Module):
    def __init__(self, args, embed_dim: int, 
        ...
        # 2. Stacked Layers (Intra + Inter + Latent Update)
        ...
        for _ in range(num_layers):
            layer_modules = nn.ModuleDict({
                ...
                # --- NEW: Latent Update inside Layer ---
                # Latent queries the current state of all nodes to update itself
                'latent_updater': SafeCrossAttnEncoder(embed_dim, num_heads=8, dropout=attn_dropout_rate)
            })
            self.layers.append(layer_modules)

        # 3. The Learnable Query Token (Patient Prototype)
        self.latents = nn.Parameter(torch.randn(1, 1, embed_dim))
    ...
    def _intra_group_step(self, 
        ...
        for g_feat, g_mask in zip(group_embeddings, group_masks):
            ...
            # Self Attention (Query=Key=Value=Group)
    ...
    def _inter_group_interaction(self, 
        ...
        # Step 1: Edge queries Source (Gather info from Source guided by Edge semantics)
        ...
        # Step 2: Target queries Gated Source (Integrate info into Target)
    ...
    def _inter_group_step_full(self, 
        ...
        for (idx_a, idx_b) in edge_keys:
            ...
            # 1. Update Edge (Contextualize based on connected nodes)
            ...
            # 2. Update Node B (using info from A via Edge)
            ...
            # 3. Update Node A (using info from B via Edge)
    ...
    def forward(
        ...
        # 0. Cleanup Knowledge Graph inputs
        ...
        # Validity Check (used for Loss later)
        ...
        # --- Initialize Latent (Patient Prototype) ---
        batch_size = group_embeddings[0].size(0)
        curr_latents = self.latents.repeat(batch_size, 1, 1)

        # 3. Graph Layers (Iterative Process)
        for layer_module in self.layers:
            # A. Intra-Group Step
            group_embeddings = self._intra_group_step(group_embeddings, group_masks, layer_module)
            
            # B. Inter-Group Step (Update Nodes & Edges)
            ...
            # C. Latent Update Step (Integrated into the loop)
            # The latent token now extracts information from the CURRENT state of all nodes
            # effectively "watching" the interaction process and updating itself.
            
            # Flatten all groups to create a global context for this layer
            global_feat = torch.cat(group_embeddings, dim=1) 
            global_mask = torch.cat(group_masks, dim=1)
            padding_mask = (global_mask == 0) # True = Padding

            # Latent (Query) attends to All Nodes (Key/Value)
            curr_latents = layer_module['latent_updater'](
                query=curr_latents, 
                key=global_feat, 
                value=global_feat, 
        ...
        # Post-Fusion Norm on the final latent
        fused_embedding = self.post_fusion_norm(curr_latents.squeeze(1))

        # --- 4. KL Loss Calculation (Unchanged) ---
        ...
        for (idx_a, idx_b) in edge_keys:
            ...
            if edge_score is None: edge_score = torch.zeros(batch_size, device=curr_latents.device)
