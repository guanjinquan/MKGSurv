...
class MedKGATFusion(nn.Module):
    def __init__(self, args, embed_dim: int, 
        ...
        # --- NEW: Iterative Latent Query Fusion Mechanism ---
        ...
        # Instead of manual components, we reuse the robust SafeCrossAttnEncoder
    ...
    def forward(
        ...
        # --- 4. Iterative Latent Query Fusion ---
        
        batch_size = group_embeddings[0].size(0)
        
        # Initialize Latent Token: (B, 1, D)
        latents = self.latents.repeat(batch_size, 1, 1)

        # Iterate through each group: Latent accumulates info from each group sequentially
        fused_order = list(range(len(group_embeddings)))
        if self.training: 
            random.shuffle(fused_order)

        for g_idx in fused_order:
            # Skip Group
            if self.drop_group_ratio > random.random():
                continue

            # Prepare key_padding_mask for current group (True = Padding)
            g_feat, g_mask = group_embeddings[g_idx], group_masks[g_idx]
            padding_mask = (g_mask == 0)
            
            # Query = Latent (Current State)
            # Key/Value = Group Features (Information Source)
            latents = self.latent_fusion_encoder(
                query=latents, 
                key=g_feat, 
                value=g_feat, 
