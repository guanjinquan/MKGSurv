...
class MedKGATFusion(nn.Module):
    def __init__(self, args, embed_dim: int, 
        ...
        # --- NEW: Dynamic Self-Guided Fusion (Robust to Cold Start) ---
        # REMOVED: self.latents (Fixed parameter is hard to train)
        
        # 1. Shared Fusion Encoder (Attention Refinement)
        self.fusion_encoder = SafeCrossAttnEncoder(embed_dim, num_heads=8, dropout=attn_dropout_rate)
        
        # 2. Learnable Gate: Controls how much the Attention Refinement contributes.
        # Initialized to 0.0 so we start exactly as "Mean Pool" baseline, then learn to add details.
        self.refinement_gate = nn.Parameter(torch.tensor([0.0]))
    ...
    def forward(
        ...
        # --- 4. Dynamic Self-Guided Fusion (Robust Strategy) ---
        
        # A. Concatenate for global context
        global_feat = torch.cat(group_embeddings, dim=1) 
        global_mask = torch.cat(group_masks, dim=1)
        padding_mask = (global_mask == 0)

        # B. Generate Dynamic Query (The "Rough Sketch" of the patient)
        # Use Mean Pool as the initial query. This guarantees a good starting point.
        dynamic_query, _ = masked_mean_pool(global_feat, global_mask)
        # dynamic_query: (B, D) -> (B, 1, D) for Attention
        dynamic_query = dynamic_query.unsqueeze(1)

        # C. Attention Refinement (The "Detail Fill-in")
        # Use the rough sketch to query the full details
        refined_delta = self.fusion_encoder(
            query=dynamic_query,
            key=global_feat,
            value=global_feat,
        ...
        # D. Gated Residual Fusion
        # Output = Baseline + Gate * Refinement
        # self.refinement_gate initialized to 0, so initially Output == Baseline
        gate_val = torch.tanh(self.refinement_gate) # Bound between -1 and 1
        fused_embedding = dynamic_query.squeeze(1) + gate_val * refined_delta

        # Safety Check
        global_all_masked = padding_mask.all(dim=1)
        if global_all_masked.any():
            fused_embedding[global_all_masked] = 0.0
        ...
        for (idx_a, idx_b) in edge_keys:
            ...
            if edge_score is None: edge_score = torch.zeros(global_feat.size(0), device=global_feat.device)
            if edge_score.dim() > 1: edge_score = edge_score.view(-1)
            if edge_score.dim() == 0: edge_score = edge_score.expand(global_feat.size(0))
