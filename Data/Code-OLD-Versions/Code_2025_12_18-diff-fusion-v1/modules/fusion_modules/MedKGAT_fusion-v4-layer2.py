...
class MedKGATFusion(nn.Module):
    def __init__(self, args, embed_dim: int, 
            ...
            num_layers: int = 2): # Default 2 layers
        super().__init__()
        ...
        # We create a ModuleList where each item is a ModuleDict containing all sub-modules for that layer
        ...
        for _ in range(num_layers):
            layer_modules = nn.ModuleDict({
                ...
                # Inter-Group: Node Update logic (Edge -> Node)
    ...
    def _intra_group_step(self, 
        ...
        """
        Apply Self-Attention within each group independently.
        """
        updated_groups = []
        
        for g_feat, g_mask in zip(group_embeddings, group_masks):
            # g_feat: (B, L, D)
            # g_mask: (B, L) - 1 valid, 0 invalid
            
            padding_mask = (g_mask == 0) # True is invalid
            ...
            processed_feat = layer_module['intra_group_transformer'](
                query=g_feat, 
                key=g_feat, 
                value=g_feat, 
            ...
            if all_masked_rows.any():
                processed_feat[all_masked_rows] = 0.0
            
            updated_groups.append(processed_feat)
    ...
    def _inter_group_interaction(self, 
        ...
        """
        Helper for One-way interaction: Source + Edge -> Target
        """
        source_padding_mask = (source_mask == 0)
        edge_padding_mask = (edge_mask == 0)
        
        # Step 1: Edge queries Source (Information Selection/Gating)
        gated_source = layer_module['edge_to_node_attn'](
            query=edge_feat, 
            key=source_node, 
            value=source_node, 
        ...
        # Step 2: Target queries Gated Source (Aggregation)
        # Note: key_padding_mask corresponds to gated_source, which has same shape as edge_feat
        updated_target = layer_module['node_to_node_attn'](
            query=target_node,
            key=gated_source,
            value=gated_source,
    ...
    def _inter_group_step_full(self, 
        ...
        """
        Process all edge interactions for the current layer.
        Returns updated group embeddings and updated edge features.
        """
        
        # Clone current state to avoid in-place modification issues during the loop
        next_group_embeddings = [g.clone() for g in group_embeddings]
        next_edges = {k: v.clone() for k, v in current_edges.items()}

        for (idx_a, idx_b) in edge_keys:
            edge_feat = current_edges.get((idx_a, idx_b))
            
            # Random Edge Dropout during training
            ...
            # Get Group Data
            feat_a = group_embeddings[idx_a] # Use input of this layer
            mask_a = group_masks[idx_a]
            feat_b = group_embeddings[idx_b] # Use input of this layer
            ...
            # --- 2. Update Node B (using A and Edge) ---
            ...
            next_group_embeddings[idx_b] = update_for_b

            # --- 3. Update Node A (using B and Edge) ---
            ...
            next_group_embeddings[idx_a] = update_for_a

        return next_group_embeddings, next_edges
    ...
    def forward(
        ...
        # 0. Clean symmetric keys (Keep only one direction i < j)
        ...
        # 1. Initial Knowledge Projection
        current_edges = {}
        for k, v in fusion_knowledge.items():
            current_edges[k] = self.know_proj(v) # Project 768 -> embed_dim

        # 2. Form Initial Group Embeddings (The "Super Nodes")
        # We concat the raw modalities into groups *once* at the beginning.
        group_embeddings = []
        group_masks = []
        ...
        # Calculate validity for Loss calculation later
        ...
        # 3. Stacked Layer Processing
        # Loop: [Intra -> Inter] * num_layers
        for layer_idx, layer_module in enumerate(self.layers):
            
            # --- A. Intra-Group Step ---
            # Refine node features based on internal context
            group_embeddings = self._intra_group_step(
                group_embeddings, 
                group_masks, 
                layer_module
            )
            ...
            # --- B. Inter-Group Step ---
            # Refine node features and edge features based on graph structure
        ...
        # 4. Global Aggregation (After all layers)
        ...
        # 5. KL Divergence Loss Calculation
        # (Uses the FINAL embeddings and FINAL edges)
        all_edge_scores_list = []
        all_valid_masks_list = []
        all_cos_sims_list = []
        edge_score_valid_flag = False

        # Pre-pool embeddings for Similarity calculation
        ...
        for (idx_a, idx_b) in edge_keys:
            ...
            # C. Cosine Similarity of Final Embeddings
        ...
        if len(all_edge_scores_list) > 0 and edge_score_valid_flag:
            ...
            all_masks_tensor = torch.stack(all_valid_masks_list, dim=1)   # (B, E)
