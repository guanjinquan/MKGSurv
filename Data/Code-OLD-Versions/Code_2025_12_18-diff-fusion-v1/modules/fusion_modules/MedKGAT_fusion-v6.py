...
class MedKGATFusion(nn.Module):
    def __init__(self, args, embed_dim: int, 
        ...
        # --- NEW: Global Latent Query Fusion Mechanism (Fix for performance drop) ---
        # 1. The Learnable Query Token (Patient Prototype)
        self.latents = nn.Parameter(torch.randn(1, 1, embed_dim))
        
        # 2. Shared Latent Fusion Encoder
        # Instead of looping, we use one robust global query to avoid order bias
        self.latent_fusion_encoder = SafeCrossAttnEncoder(embed_dim, num_heads=8, dropout=attn_dropout_rate)
    ...
    def forward(
        ...
        # --- 4. Global Latent Query Fusion (Parallel / Global Context) ---
        # FIX for performance drop: Avoid Sequential Loops which cause recency bias.
        # Instead, concatenate all groups and let Latent query the best info globally.
        
        batch_size = group_embeddings[0].size(0)
        
        # A. Concatenate all group features to form Global Context
        # group_embeddings: List of (B, Li, D) -> global_feat: (B, Sum_Li, D)
        global_feat = torch.cat(group_embeddings, dim=1) 
        global_mask = torch.cat(group_masks, dim=1)
        
        # B. Prepare Mask (1=valid, 0=padding -> True=padding)
        padding_mask = (global_mask == 0)
        
        # Note: SafeCrossAttnEncoder handles the "All Masked Rows" safety check internally
        
        # C. Initialize Latent (B, 1, D)
        latents = self.latents.repeat(batch_size, 1, 1)

        # D. Single Global Query
        # This allows the model to weigh Group A vs Group B dynamically (Global Softmax)
        fused_embedding = self.latent_fusion_encoder(
            query=latents, 
            key=global_feat, 
            value=global_feat, 
        ...
        # Global Safety Check: If a patient has NO valid data in ANY group
        ...
        for (idx_a, idx_b) in edge_keys:
            ...
            if edge_score is None: edge_score = torch.zeros(batch_size, device=latents.device)
