...
# 2. CONFIGURATION
# -------------------------------------------------------------------------
BASE_DIR = "/home/Guanjq/NewWork/MedAlignFusion/Data/TCGA-BRCA/"
CONF = {
    'h5_dir': f'{BASE_DIR}/h5_files',
    'csv_path': f'{BASE_DIR}/processed/brca_patient_labels.csv',
    'folds_json': f'{BASE_DIR}/processed/brca_patients_5fold.json',
        ...
        for pid, fpaths in tqdm(file_mapping.items(), desc="Extracting Features"):
            try:
                ...
                for fpath in fpaths:
                    ...
                    patient_embeddings.append(embedding_np)
                
                # Concatenate embeddings from multiple slides instead of averaging
                if patient_embeddings:
                    # Concatenate along the first axis (n_proto dimension)
                    # If we have 2 slides with shape (128, 2*D+1) each,
                    # the result will have shape (256, 2*D+1)
                    concatenated_embedding = np.concatenate(patient_embeddings, axis=0)
                    # pid here comes from the key of file_mapping
                    results[pid] = concatenated_embedding
...
def main():
    ...
    # Convert to single file mapping for backward compatibility
    # Take first file for each patient
    pid_to_file = {k: v[0] for k, v in pid_to_files.items() if v}
    sid_to_file = {k: v[0] for k, v in sid_to_files.items() if v}
    ...
    for fold_info in folds_data['folds']:
        ...
        # -----------------------------------------------------------
        # KEY CHANGE: Use sid_to_files instead of sid_to_file to process all files
