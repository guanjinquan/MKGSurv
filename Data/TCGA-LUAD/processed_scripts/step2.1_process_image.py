...
# 2. CONFIGURATION
# -------------------------------------------------------------------------
BASE_DIR = "/home/Guanjq/NewWork/MedAlignFusion/Data/TCGA-LUAD/"
CONF = {
    'h5_dir': f'{BASE_DIR}/h5_files',
    'csv_path': f'{BASE_DIR}/processed/luad_patient_labels.csv',
    'folds_json': f'{BASE_DIR}/processed/luad_patients_5fold.json',
...
def get_file_mapping(csv_path, h5_dir):
    ...
    Returns:
        pid_to_file: { case_id (UUID): h5_path }
        sid_to_file: { submitter_id (e.g. TCGA-XX-XXXX): h5_path }
    """
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df)):
        ...
        if matched_file:
            pid_to_file[case_id] = matched_file
            sid_to_file[submitter_id] = matched_file

    print(f"Mapped {len(sid_to_file)} patients.")
    return pid_to_file, sid_to_file
...
def extract_features(model, file_mapping, device, h5_key):
    """Runs inference on ALL files using the trained model."""
    results = {}
    model.eval()
    
    # file_mapping key should be the Submitter ID (TCGA-XX-XXXX)
    with torch.no_grad():
        for pid, fpath in tqdm(file_mapping.items(), desc="Extracting Features"):
            try:
                with h5py.File(fpath, 'r') as f:
                ...
                # Forward
                # Model output shape: (B, n_proto, 2*D + 1)
                embedding = model(x, mask)
                
                # Squeeze batch dimension (B=1)
                # Final Result Shape: (n_proto, 2*D + 1)
                embedding_np = embedding.squeeze(0).cpu().numpy()
                
                # pid here comes from the key of file_mapping
                results[pid] = embedding_np
...
def main():
    ...
    # 1. Prepare Mappings
    # pid_to_file: UUID -> Path
    # sid_to_file: SubmitterID (TCGA-XX-XXXX) -> Path
    pid_to_file, sid_to_file = get_file_mapping(CONF['csv_path'], CONF['h5_dir'])
    ...
    for fold_info in folds_data['folds']:
        ...
        for sid in train_sids:
            if sid in sid_to_file:
                train_files.append(sid_to_file[sid])
            else:
                missing_train += 1
        ...
        # -----------------------------------------------------------
        # KEY CHANGE: Use sid_to_file instead of pid_to_file
        # This ensures the output dictionary keys are TCGA-XX-XXXX
        fold_features = extract_features(model, sid_to_file, CONF['device'], CONF['h5_feature_key'])
