...
class SafeCrossAttnEncoder(nn.Module):
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, 
        ...
        key = self.norm_attn(key)
        value = self.norm_attn(value)
        
        # --- Safe Logic ---
        if key_padding_mask is not None:  # 检测哪些样本的所有 Key 都是 Padding
...
class MedKGATFusion(nn.Module):
    def __init__(self, args, embed_dim: int, 
        ...
        self.modalities_embeddings = nn.Embedding(max_modalities, embed_dim)
    ...
    def forward(
        ...
        # 2. Add Modalities Embeddings
        device = next(self.parameters()).device
        for i in range(len(embeddings)):
            if embeddings[i] is None:
                continue
            mod_idx = torch.tensor(i, device=device)
            mod_emb = self.modalities_embeddings(mod_idx).unsqueeze(0).unsqueeze(0)
            embeddings[i] = (embeddings[i] + mod_emb) * masks[i].unsqueeze(-1)  

        # 3. Intra-Group Interaction (Multi-layer handled inside TransformerEncoder)
        info_level_embeddings = self._intra_group_step(embeddings, masks, embeddings_groups)

        # 4. Create Group-Level Embeddings
        group_embeddings = []
        group_masks = []
        ...
        # 5. Inter-Group Interaction (Multi-Layer GNN / GAT)
        ...
        # ------------------------------------------------------------------
        # 5b. Data Collection for KL Loss (Post-GAT)
