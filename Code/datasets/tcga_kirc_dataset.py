...
class TCGA_KIRC_Dataset(MultiModalDataset):
    ...
    def __init__(self, args, mode: str = "train", modalities: str = "all", fold: int = None):
        ...
        if args.knowledge_type == 'all':
            self.dataset_dir = os.path.join(project_root, "Data", "TCGA-KIRC")
        elif args.knowledge_type == 'relationship':
            self.dataset_dir = os.path.join(project_root, "Data-Correlation", "TCGA-KIRC")
        elif args.knowledge_type == 'survival':
            self.dataset_dir = os.path.join(project_root, "Data-Survival", "TCGA-KIRC")
        else:
            raise ValueError(f"Invalid knowledge type: {args.knowledge_type}")
        ...
        split_file = os.path.join(self.processed_dir, "kirc_patients_5fold.json") 
    ...
    def _load_tabular_and_labels(self):
        try:
            ...
            self.labels_df = pd.read_csv(os.path.join(self.processed_dir, "kirc_patient_labels.csv"), dtype=str)
        except Exception as e:
        ...
        if "tabular-clinical-6" in self.modalities:
        ...
        if "tabular-pathology-20" in self.modalities:
