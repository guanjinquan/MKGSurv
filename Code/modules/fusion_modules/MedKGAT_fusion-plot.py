...
class MedKGATFusion(nn.Module):
    ...
    def forward(
        ...
        for (idx_a, idx_b), edge_feat in proj_knowledge.items():
            ...
            if edge_mask is None:  # Fallback if mask missing
            ...
            if edge_score is None:
                edge_score = torch.tensor(0.0, device=edge_feat.device)
            edge_score = edge_score.view(-1, 1, 1)
            ...
            # weight update with edge score
            weight_for_b = weight_for_b # * edge_score
            weight_for_a = weight_for_a # * edge_score
        ...
        if all_masked_rows.any():  # Handle strictly or clone/unmask
