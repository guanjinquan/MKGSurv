...
# --- Main HierAlign Fusion Module ---
class HierAlignFusionModule(nn.Module):
    """
    I²MoE fusion module, adapted to handle a dynamic number of modalities with token-level masks.
    """

    def __init__(self, embed_dim: int, max_modalities: int, num_layers: int = 2, num_heads: int = 8, dropout: float = 0.1):
        super(HierAlignFusionModule, self).__init__()
        
        # --- Step 1: Initialize variables ---
        self.max_modalities = max_modalities
        self.embed_dim = embed_dim
        
        # --- Step 2: Initialize align module ---
        self.align_module = AlignmentModule(embed_dim=embed_dim)

        # --- Step 3: Initialize fusion experts ---
    ...
    def _diversity_loss(self, expert_outputs: List[List[torch.Tensor]], present_indices: List[int]) -> torch.Tensor:
        """
        expert_outputs: list over experts (num_branches), each elem is list of outputs from forward_multiple.
                        primary output per expert: expert_outputs[e][0] -> shape (B, D)
        present_indices: indices of present modalities (w.r.t uniqueness experts)
        """
        device = next(self.parameters()).device
        primary_embeddings = [output[0] for output in expert_outputs]  # list of (B, D), len = num_branches

        # Split groups
        uniqueness_experts = primary_embeddings[: self.max_modalities]  # list len = max_modalities
        synergy_expert = primary_embeddings[self.max_modalities]        # (B, D)
        redundancy_expert = primary_embeddings[self.max_modalities + 1] # (B, D)

        losses = []
        # 1) uniqueness vs uniqueness -> minimize similarity (push cos -> 0)
        if len(present_indices) > 1:
            for i_idx in range(len(present_indices)):
                for j_idx in range(i_idx + 1, len(present_indices)):
                    i = present_indices[i_idx]
                    j = present_indices[j_idx]
                    cos_ij = torch.mean(uniqueness_experts[i] * uniqueness_experts[j], dim=1)  # (B,), 取均值，不然loss会很大
                    losses.append((cos_ij ** 2).mean())

        # 2) uniqueness vs synergy -> minimize similarity (push cos -> 0) because synergy is emergent
        for i in present_indices:
            cos_is = torch.mean(uniqueness_experts[i] * synergy_expert, dim=1)  # (B,)
            losses.append((cos_is ** 2).mean())

        # 3) uniqueness vs redundancy -> maximize similarity (push cos -> 1)
        for i in present_indices:
            cos_ir = torch.mean(uniqueness_experts[i] * redundancy_expert, dim=1)  # (B,)
            losses.append(((1.0 - cos_ir) ** 2).mean())
    ...
    def forward(self, embeddings: List[Optional[torch.Tensor]], masks: List[Optional[torch.Tensor]], align_pairs: List[Tuple[int, int]], **kargs) -> Dict:
        """
        ...
        if num_present == 0:
            raise ValueError("No embeddings present for fusion and final prediction")

        # Step 1: Align embeddings in `align_pairs` via contrastive loss
        align_losses = {}
        ...
        # Step 2: Fuse embeddings then masked mean pool
        ...
        # --- Loss Calculation ---
        # (1) compute I2MoE interaction losses
        interaction_losses = {}
        ...
        # (2) compute cosine similarity loss among uniqueness_experts, synergy_expert, redundancy_expert
        # Loss_diversity = (1 - cos(uni, uni)) + (1 - cos(uni, syn)) + cos(uni, red)
        # interaction_losses["diversity"] = self._diversity_loss(expert_outputs, present_indices)

        # --- Re-weighting and Fusion ---
        all_primary_embeddings = torch.stack([output[0] for output in expert_outputs], dim=1)
        interaction_weights = self.reweight(full_embeddings, full_masks)   # (B, num_branches)
        fused_embedding = (all_primary_embeddings * interaction_weights.unsqueeze(2)).sum(dim=1)

        # Collect align losses and interaction losses
        fusion_losses_dict = {f"align_{k}": v for k, v in align_losses.items()}
        fusion_losses_dict.update({f"interaction_{k}": v for k, v in interaction_losses.items()})
        fusion_losses_dict['total'] = sum(l for l in fusion_losses_dict.values() if isinstance(l, torch.Tensor))

        return {"fused_embedding": fused_embedding, "loss_dict": fusion_losses_dict}
