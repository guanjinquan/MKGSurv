...
class MedKGATFusion(nn.Module):
    ...
    def forward(
        ...
        # Pre-calculate pooled embeddings for cosine sim (B, D)
        # These are the embeddings *before* the GNN update, which is usually correct 
        # for regularizing the initial structure, but can be done after if preferred.
        # Assuming we want to regularize the structure learned so far:
        
        # [FIX]: Robust handling of masked_mean_pool return type
        raw_pooled = [masked_mean_pool(g, m) for g, m in zip(group_embeddings, group_masks)]
        pooled_group_embeddings = []
        for x in raw_pooled:
            if isinstance(x, tuple):
                pooled_group_embeddings.append(x[0]) # Assuming first element is the embedding
            else:
                pooled_group_embeddings.append(x)
        
        # Normalize for cosine similarity
        pooled_group_embeddings = [F.normalize(g, p=2, dim=1) for g in pooled_group_embeddings]
        ...
        for (idx_a, idx_b), edge_feat in proj_knowledge.items():
            ...
            has_a = (mask_a.sum(dim=1) > 0).float()
            has_b = (mask_b.sum(dim=1) > 0).float()
            
            # Pair validity for this patient
            pair_validity = has_a * has_b # (B,)

            weight_for_b = has_a.view(-1, 1, 1) * edge_score.view(-1, 1, 1)
            ...
            # --- Collect Data for Loss ---
            # Compute Cosine Similarity for this pair (B,)
            # pooled_group_embeddings are already normalized
            sim = torch.sum(pooled_group_embeddings[idx_a] * pooled_group_embeddings[idx_b], dim=1)
            
            # Clamp similarity to [-1, 1] for numerical stability
            sim = torch.clamp(sim, -1.0, 1.0)
            
            # Since KL divergence usually works with positive probabilities, 
            # and edge scores are likely positive (e.g., 1, 9, 100),
            # we map Cosine Sim [-1, 1] to a strictly positive range for distribution matching.
            # Common mapping: (sim + 1) / 2 -> [0, 1]. Or use exp(sim / temp).
            # Here we keep raw sim and handle distribution mapping via Softmax later.
        ...
        # # =========================================================================
        # # 6. Deep Supervision & Data Saving (FIXED SHAPES)
        # # =========================================================================
        
        # # --- 1. 预先计算 Mean Pooling ---
        # group_mean_embeddings = []
        # for i in range(len(final_group_embeddings)):
        #     res = masked_mean_pool(final_group_embeddings[i], group_masks[i])
        ...
        # for (idx_a, idx_b), _ in proj_knowledge.items():
        #     # A. 获取 Edge Score
        ...
        #     save_points_path = "/home/Guanjq/NewWork/MedAlignFusion/Data/TCGA-LUAD/llm_sim/points.jsonl"
        #     # 确保目录存在
        #     try:
        ...
        # [FIX] Also check tuple return here just in case
        ...
        # --- 7. Compute KL Divergence Loss (Patient-wise) ---
        fusion_loss = torch.tensor(0.0, device=fused_embedding.device)
        
        # [DEBUG PRINT START]
        # if True: # Switch to False to turn off prints in production
        #     print(f"\n[KL_DEBUG] Total Edges in Graph: {len(all_edge_scores_list)}")
            
        if len(all_edge_scores_list) > 0:
            ...
            # # [DEBUG PRINT]
            # if True:
            #     num_valid_edges_per_patient = all_masks_tensor.sum(dim=1)
            #     print(f"[KL_DEBUG] Valid Edges per patient (Batch first 4): {num_valid_edges_per_patient[:4].tolist()}")
            #     print(f"[KL_DEBUG] Max Edge Score: {all_scores_tensor.max().item()}, Min Sim: {all_sims_tensor.min().item()}")
            ...
            # Ensure non-negative if scores can be negative (though usually edge weights are >0)
            # Scale scores if necessary (e.g. temperature)
            ...
            # # [DEBUG PRINT]
            # if True:
            #      print(f"[KL_DEBUG] Target Probs (Patient 0, first 5): {target_probs[0, :5].detach().cpu().numpy()}")
            #      print(f"[KL_DEBUG] Pred LogProbs (Patient 0, first 5): {pred_log_probs[0, :5].detach().cpu().numpy()}")
            ...
            #     # [DEBUG PRINT]
            #     if True:
            #         print(f"[KL_DEBUG] Avg Loss: {fusion_loss.item():.6f} (Computed over {int(valid_patients.sum().item())} valid patients)")
            # else:
            #     # [DEBUG PRINT]
            #     if True:
            #         print(f"[KL_DEBUG] Avg Loss: 0.0 (NO valid patients with >1 edge found!)")
