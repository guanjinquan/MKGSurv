...
class FeedForward(nn.Module):
    def __init__(self, dim, mult = 4, dropout = 0.25):
        super().__init__()
...
class SafeCrossAttnEncoder(nn.Module):
    ...
    包含了防 NaN 的安全机制。
    """
    def __init__(self, embed_dim: int, num_heads: int = 8, dropout: float = 0.25, ffn_mult: int = 4):
        super().__init__()
...
class MedKGATFusion(nn.Module):
    def __init__(self, args, embed_dim: int, max_modalities: int = 10, dropout_rate: float = 0.25, 
    ...
    def forward(
        ...
        # 7. Global 
        final_aggregated_group_embeddings = []
        final_aggregated_group_masks = []
        for g, m in zip(final_group_embeddings, group_masks):
            embed, mask = masked_mean_pool(g, m)
            final_aggregated_group_embeddings.append(embed)
            final_aggregated_group_masks.append(mask)

        global_concat = torch.stack(final_aggregated_group_embeddings, dim=1)
        global_mask = torch.stack(final_aggregated_group_masks, dim=1)
