...
def GetOptimizer(args, model):
    ...
    # 使用 id 区分 backbone 与 others（更可靠）
    ...
    backbone_trainable_mb = params_size_mb(backbone_params, only_trainable=True)
    other_trainable_mb = params_size_mb(other_params, only_trainable=True)
    total_trainable_mb = params_size_mb(all_params, only_trainable=True)

    # 打印 summary
    print(f"[PARAMS] Trainable params (MB): total={total_trainable_mb:.3f} MB; backbone={backbone_trainable_mb:.3f} MB; others={other_trainable_mb:.3f} MB")
    ...
    backbone_group = filter_trainable(backbone_params)
    other_group = filter_trainable(other_params)

    # Fallback: if no backbone detected, just optimize all trainable params as a single group
    if args.optimizer == 'Adam':
        if backbone_group:
            optimizer = torch.optim.Adam([
                {'params': backbone_group, 'lr': getattr(args, 'backbone_lr', args.learning_rate), 'weight_decay': args.weight_decay},
                {'params': other_group, 'lr': args.learning_rate, 'weight_decay': args.weight_decay},
            ])
        else:
            optimizer = torch.optim.Adam([
                {'params': filter_trainable(all_params), 'lr': args.learning_rate, 'weight_decay': args.weight_decay},
            ])
